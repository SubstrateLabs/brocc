...
from brocc_li.utils.html_metadata import HtmlMetadata, extract_metadata
...
class TestHtmlMetadata:
    ...
    def test_enhanced_author_extraction(self):
        """Test extraction of author information from additional formats"""
        # Test schema.org with meta itemprop
        html_schema_meta = (
            """<html><head><meta itemprop="author" content="Schema Meta Author"></head></html>"""
        )
        metadata_schema_meta = extract_metadata(html_schema_meta)
        assert metadata_schema_meta.author == "Schema Meta Author"

        # Test schema.org with span itemprop
        html_schema_span = (
            """<html><body><span itemprop="author">Schema Span Author</span></body></html>"""
        )
        metadata_schema_span = extract_metadata(html_schema_span)
        assert metadata_schema_span.author == "Schema Span Author"

        # Test schema.org with nested name
        html_schema_nested = """
        <html><body>
        <span itemprop="author">
            <span itemprop="name">Schema Nested Author</span>
        </span>
        </body></html>
        """
        metadata_schema_nested = extract_metadata(html_schema_nested)
        assert metadata_schema_nested.author == "Schema Nested Author"

        # Test Dublin Core
        html_dc = (
            """<html><head><meta name="DC.creator" content="Dublin Core Author"></head></html>"""
        )
        metadata_dc = extract_metadata(html_dc)
        assert metadata_dc.author == "Dublin Core Author"

        # Test rel="author" link with title
        html_rel_link = (
            """<html><head><link rel="author" title="Link Author" href="/author"></head></html>"""
        )
        metadata_rel_link = extract_metadata(html_rel_link)
        assert metadata_rel_link.author == "Link Author"

        # Test a rel="author"
        html_rel_a = """<html><body><a rel="author">A Tag Author</a></body></html>"""
        metadata_rel_a = extract_metadata(html_rel_a)
        assert metadata_rel_a.author == "A Tag Author"

        # Test byline class
        html_byline = """<html><body><div class="byline">Byline Author</div></body></html>"""
        metadata_byline = extract_metadata(html_byline)
        assert metadata_byline.author == "Byline Author"

        # Test byline with prefix
        html_byline_prefix = (
            """<html><body><div class="author">By Author With Prefix</div></body></html>"""
        )
        metadata_byline_prefix = extract_metadata(html_byline_prefix)
        assert metadata_byline_prefix.author == "Author With Prefix"

        # Test JSON-LD with string author
        html_jsonld_string = """
        <html><head>
        <script type="application/ld+json">
        {
            ...
            "author": "JSON-LD String Author"
        }
        </script>
        </head></html>
        """
        metadata_jsonld_string = extract_metadata(html_jsonld_string)
        assert metadata_jsonld_string.author == "JSON-LD String Author"

        # Test JSON-LD with object author
        html_jsonld_object = """
        <html><head>
        <script type="application/ld+json">
        {
            ...
            "author": {
                "@type": "Person",
                "name": "JSON-LD Object Author"
            }
        }
        </script>
        </head></html>
        """
        metadata_jsonld_object = extract_metadata(html_jsonld_object)
        assert metadata_jsonld_object.author == "JSON-LD Object Author"
